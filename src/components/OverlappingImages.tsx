  ...
  return (
    <div style={{width: `${(size  + ((limitedImages.length-1) * (size* (1-ratio))))+outline}px`, height: `${size+outline}px`}} className='flex relative justify-start items-center'>
        {limitedImages.reverse().map((image,index, arr) => (
          <div key={index} style={{left:`${index === 0? 0: (index)*(size+outline)*(1-ratio)}px`, width:`${size}px`, zIndex:arr.length-index-1, height:`${size}px`}} className='absolute rounded-circle outline-[var(--overlapping-outline-color)] outline outline-[2px] overflow-hidden'>
